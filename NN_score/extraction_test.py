...
for numeric in range(0, 4000):
    ...
    for take_ml in tuples:
        ...
        if len(test) == 1000:
            ...
            with open('/home/nadia/data/test/{}test.pickle'.format(max(NUMBER2) + 1), 'wb') as f:
                dump(test, f)
        ...
        if numeric == 4000 and take_ml == tuples[-1]:
            ...
            with open('/home/nadia/data/test/{}test.pickle'.format(max(NUMBER2)+1), 'wb') as f:
                dump(test, f)
                test.clear()
with open('/home/nadia/data/sig_test/sig_test.pickle') as f:
    dump(sig_test, f)
fh.setFormatter(formatter)
logger.addHandler(fh)
...
for numeric in range(4000, 10000):
    ...
    for take_ml in tuples:
        ...
        if len(train) == 8000:
            ...
            with open('/home/nadia/data/train/{}train.pickle'.format(NUMBER3), 'wb') as f:
                dump(train, f)
            fh.setFormatter(formatter)
            logger.addHandler(fh)
            logger.info(f"train : {(len(NUMBER3)+1)*8000}")
            train.clear()
        if numeric == 10000 and take_ml == tuples[-1]:
            NUMBER3.add(max(NUMBER3) + 1)
            with open('/home/nadia/data/train/{}train.pickle', 'wb') as f:
                dump(train, f)
            train.clear()
